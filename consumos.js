// ===============================================
// CONSUMOS MENSUALES - v2 (limpio) [FIX MESES]
// ===============================================

// Helper seguro para YYYY-MM (evita líos de zona horaria con toISOString)
function monthKey(d) {
...
function renderConsumos() {
  const [year, month] = currentConsumosMes.split('-').map(Number);
  const mesLabel = new Date(year, month - 1).toLocaleDateString('es-ES', {
    month: 'long',
    year: 'numeric'
  });
...
function renderConsumosChart() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const key = monthKey(d); // ✅ FIX aquí también
